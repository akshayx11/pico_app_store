...
class TOTPApp:
    ...
    def draw_pin_screen(self):
        ...
        display_str = f"PIN: {stars}{spaces}\n\n[#]/[A]: Submit\n[C]: Back  [D]: Exit"
        self.mv.show_screen(display_str, "Master PIN", 5, 22)
    ...
    def start(self):
        ...
        self.mv.show_screen("[A]: Sync NTP Time\n[B]: Skip (Use RTC)\n[D]: Exit App", "Time Sync", 5, 22)
        self.keypad.key_functions = self.keypad.default_key_mappings()
        ...
        self.keypad.key_functions["B"] = choose_skip
        self.keypad.key_functions["D"] = choose_exit
        
        while choice is None and self.is_running:
            self.keypad.process_keypress(True)
            time.sleep(0.05)
            
        if choice == "exit" or not self.is_running:
            return
        elif choice == "sync":
            self.sync_time()
